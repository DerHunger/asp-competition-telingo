%*
telingo encoding for Sokoban problem from the ASP Competition 2013
...
#program initial.
dir(dir_right). dir(dir_left). dir(dir_up). dir(dir_down).

inverseDir(dir_right,dir_left). inverseDir(dir_left,dir_right).
inverseDir(dir_up,dir_down). inverseDir(dir_down,dir_up).

% dead position:
% a stone is stuck and can't be moved away
% corner, 2 movedir which are not inverse directions
deadPos(Pos1) :- isnongoal(Pos1), 2 = #count{Dir : movedir(Pos1,Pos2,Dir)},
                    movedir(Pos1,_,Dir1), movedir(Pos1,_,Dir2), Dir1 != Dir2, not inverseDir(Dir1,Dir2).
% dip, 1 movedir (3 walls)
deadPos(Pos1) :- isnongoal(Pos1), 1 = #count{Dir : movedir(Pos1,Pos2,Dir)}.
...
deadWall(Pos2,Dir) :- isnongoal(Pos2), deadPos(Pos1), deadComb(Pos1,Pos2), movedir(Pos1,Pos2,Dir).
